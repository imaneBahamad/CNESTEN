				...
				<table class="table table-bordered table-striped table-condensed">
					...
					<tbody>
						<c:forEach items="${ sessionScope.recherche }" var="mapRecherche" varStatus="boucle">
							<tr>
								<td><c:out value="${ mapRecherche.value.action.nom_action}"/></td>
								<td>
									<span style="font-size:1.3em; color:Tomato"><i class="fas fa-file-pdf"></i></span>&ensp;
									<a href="<c:url value="/downloadFiche"><c:param name="idFiche" value="${mapRecherche.key }" /></c:url>">
										<c:out value="${mapRecherche.value.nom_fiche}"/>
									</a>
								</td>
								<c:if test='${sessionScope.sessionUtilisateur.privilège=="Administrateur" }'>							
									<td>
										<a href="<c:url value="/modificationFiche"><c:param name="idFiche" value="${mapRecherche.key }" /></c:url>">
											<i class="fas fa-edit"></i>
										</a>&emsp;
										<a href="<c:url value="/suppressionFiche"><c:param name="idFiche" value="${mapRecherche.key }" /></c:url>">
