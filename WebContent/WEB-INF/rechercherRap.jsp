						...
						<c:forEach items="${ sessionScope.recherche }" var="mapRecherche" varStatus="boucle">
							<tr>
								...
								<td>
									...
									<a href="<c:url value="/downloadRap"><c:param name="idRap" value="${mapRecherche.key }" /></c:url>">
										<c:out value="${mapRecherche.value.nom_rap}"/>
									</a>
								</td>
								<c:if test='${sessionScope.sessionUtilisateur.privilège=="Administrateur" }'>						
									<td>
										<a href="<c:url value="/modificationRap"><c:param name="idRap" value="${mapRecherche.key }" /></c:url>">
											<i class="fas fa-edit"></i>
										</a>&emsp;
										<a href="<c:url value="/suppressionRap"><c:param name="idRap" value="${mapRecherche.key }" /></c:url>">
